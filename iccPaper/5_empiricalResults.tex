...
% The probability of interest is the probability of being in a category given that you have progressed to that stage. The cut point equations are estimated simultaneously, which result in a computational economy.

This modeling framework assumes that $Y$ arises from a latent continuous variable $\widetilde{Y}$. To model the categorization process we assume that there $K$ thresholds, $\tau_{k}$, that partition $\widetilde{Y}$ into $K+1$ observable ordered categories of $Y$.  

the dependent variable is split up into $K-1$ logit equations corresponding to, in our case, the different phases of ICC involvement. The equation for the probability of being in a certain phase of the ICC process given that a country has progressed to that stage is:
...
$F$ denotes the logistic CDF. $\tau_{k}$

$\tau$ is a cut point

$\beta$ is a vector of logit coefficents for a matrix of independent variables $X$

In its most general form, as proposed by \citet{tutz1990sequential}, for every category $k \in \{1, \ldots, K\}$ there is a latent continuous variable, $\widetilde{Y_{k}}$, that sequentially determines the transition between categories. In our case, $\widetilde{Y_{k}}$ represents all the factors contributing to the probability of a country proceeding beyond a given ICC investigation stage.

More specifically, we follow \citet{tutz1990sequential} and \citet{agresti:2010} in utilizing a latent variable framework. For $y=1,\ldots,K$, there are $K-1$ latent variables.

%% need to make sure i dont confuse notation for continuation vs stopping model, we are using a continuation here
% in this section i've laid out basic motivation for sequential but now we want ot lay out the model (so figure out notation)
% then make sure to discuss that you let coefficients vary acros sstages so you have hte partial CR model
...
% Informally, we could call $\tilde{Y_{k}}$ ``suspicious level'' in the ongoing example.
