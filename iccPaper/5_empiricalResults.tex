...
In the continuation ratio framework, the dependent variable is split up into $K-1$ logit equations corresponding to, in our case, the different phases of ICC involvement.



To formalize this model, we follow \citet{tutz1990sequential} and \citet{agresti:2010} in utilizing a latent variable framework.


For $y=1,\ldots,K$, there are $K-1$ latent variables. Each latent variable, $z_{m}$, corresponds to a particular cutpoint equation, for which the structural model can be expressed as:
