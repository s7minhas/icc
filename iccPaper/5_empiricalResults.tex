% 31, 64, 92, 142
% unconstrained partial continuation ratio model
...
% Since the thresholds $\tau_{k}$ refer to different latent variables, they don't need to be ordered. That is, we may as well have $\tau_{k+1} < \tau_{k}$. Similar to standard cumulative ordinal models, we need to assume a certain distribution for $\tilde{Y_{k}}$ with cumulative distribution function $F$. Let's suppose we want to model the probability of a country facing a formal investigation. This means that 
