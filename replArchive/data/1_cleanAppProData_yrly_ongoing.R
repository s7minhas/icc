...
data = apData %>% 
	...
	dplyr::summarize(
		...
		icclevel = max(icclevel, na.rm=TRUE),
		icclevel_state = max(icclevel_state, na.rm=TRUE),
		icclevel_opp = max(icclevel_opp, na.rm=TRUE),				
...
save(data, file=paste0(pathData, 'baseData_yrly_ongoing.rda'))
###############################################################
