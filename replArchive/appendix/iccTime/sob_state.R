...
tKey$iccTime = 1:nrow(tKey)
data$iccTime = tKey$iccTime[match(data$year, tKey$year)]

#
sobStateVars = c(sobStateVars, 'iccTime')
###############################################################
...
save(
  mod, 
  file=paste0(pathResults, 'sobState_iccTime_fin.rda'))
###############################################################