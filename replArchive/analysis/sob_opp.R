...
if(!file.exists(paste0(pathData, 'sobOpp_imp_fin.rda'))){
	toImp = data.matrix(data[,c('icclevel_opp',sobOppVars)])
	impData = sbgcop.mcmc(Y=toImp, seed=6886, verb=FALSE, nsamp=1000)
	save(impData, file=paste0(pathData, 'sobOpp_imp_fin.rda'))
} else { load(paste0(pathData, 'sobOpp_imp_fin.rda')) }
...
save(
	mod, 
	file=paste0(pathResults, 'sobOpp_model1a_1_newp5Var_fin.rda'))
###############################################################