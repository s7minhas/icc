...
frame = cbind(data[,c('ccode','year','icclevel2_opp_3b')], frame)
frame$icclevel2_opp_3b = as.integer(frame$icclevel2_opp_3b)
frame$ccode = as.integer(frame$ccode)
frame = na.omit(frame)
impDFs = lapply(sample(500:1000, 10), function(i){
	x = data.frame(impData$Y.impute[,,i])
	x = cbind(data[,c('ccode','year','icclevel2_opp_3b')], x)
	names(x) = names(frame)
	x$icclevel2_opp_3b = as.integer(x$icclevel2_opp_3b)
...
save(mod, file=paste0(pathResults, 'sobOpp_model2b_global.rda'))
...
save(modHier, file=paste0(pathResults, 'sobOpp_model2b_global_hier.rda'))
###############################################################