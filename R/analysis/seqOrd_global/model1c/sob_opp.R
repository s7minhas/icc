...
frame = cbind(data[,c('ccode','year','icclevel_opp_4b')], frame)
frame$icclevel_opp_4b = as.integer(frame$icclevel_opp_4b + 1)
frame$ccode = as.integer(frame$ccode)
impDFs = lapply(sample(500:1000, 10), function(i){
	x = data.frame(impData$Y.impute[,,i])
	x = cbind(data[,c('ccode','year','icclevel_opp_4b')], x)
	names(x) = names(frame)
	x$icclevel_opp_4b = as.integer(x$icclevel_opp_4b + 1)
...
save(mod, file=paste0(pathResults, 'sobOpp_model1c_global.rda'))
...
save(modHier, file=paste0(pathResults, 'sobOpp_model1c_global_hier.rda'))
###############################################################