...
frame = cbind(data[,c('ccode','year','icclevel_state')], frame)
...
impDFs = lapply(sample(500:1000, 10), function(i){
	...
	x = cbind(data[,c('ccode','year','icclevel_state')], x)
	names(x) = names(frame)
...
save(mod, file=paste0(pathResults, 'sobState_model0_global.rda'))
...
save(modHier, file=paste0(pathResults, 'sobState_model0_global_hier.rda'))
###############################################################