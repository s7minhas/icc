...
frame = cbind(data[,c('ccode','year','icclevel_opp')], frame)
...
impDFs = lapply(sample(500:1000, 10), function(i){
	...
	x = cbind(data[,c('ccode','year','icclevel_opp')], x)
	names(x) = names(frame)
...
save(mod, file=paste0(pathResults, 'sobOpp_model0_global.rda'))
...
save(modHier, file=paste0(pathResults, 'sobOpp_model0_global_hier.rda'))
###############################################################