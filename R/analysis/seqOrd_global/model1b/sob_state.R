...
frame = cbind(data[,c('ccode','year','icclevel_state_4a')], frame)
frame$icclevel_state_4a = as.integer(frame$icclevel_state_4a + 1)
frame$ccode = as.integer(frame$ccode)
impDFs = lapply(sample(500:1000, 10), function(i){
	x = data.frame(impData$Y.impute[,,i])
	x = cbind(data[,c('ccode','year','icclevel_state_4a')], x)
	names(x) = names(frame)
	x$icclevel_state_4a = as.integer(x$icclevel_state_4a + 1)
...
save(mod, file=paste0(pathResults, 'sobState_model1b_global.rda'))
...
save(modHier, file=paste0(pathResults, 'sobState_model1b_global_hier.rda'))
###############################################################