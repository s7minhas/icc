...
###############################################################
# summarize
load(paste0(pathResults, 
	'sobOpp_model1a_global_iccRatOSV_interaction.rda'))
oppMod = mod
load(paste0(pathResults, 
	'sobState_model1a_global_iccRatOSV_interaction.rda'))
stateMod = mod
...
sobOppVars = c(
	...
	'lag1_gdpCapLog','africa',
	'lag1_v2juncind',
	'lag1_osv_rebel_cumul',	'iccRat_osv',
	# p5 vars: 
	'lag1_p5_absidealdiffMin',
	'lag1_p5_defAllyMax',
	'lag1_p5_gov_clean', 'lag1_p5_reb_clean'
...
sobStateVars = c(
	...
	'lag1_gdpCapLog','africa',
	'lag1_v2juncind','lag1_pts',
	'lag1_osv_state_cumul',	'iccRat_osv',
	# p5 vars: 
	'lag1_p5_absidealdiffMin',
	'lag1_p5_defAllyMax',
	'lag1_p5_gov_clean', 'lag1_p5_reb_clean'
...
sobVars = unique(c(sobStateVars, sobOppVars))
sobVars = sobVars[c(1:9,14,10:13)]
sobVars = c(
	sobVars[c(1:4,6:7,12:14)],
	sobVars[c(5,8,10,9,11)]
	)
...
print.xtable(xtable(res, align='llcc', caption=lab),
	...
	file=paste0(pathResults, 
		'sob_model1a_global_iccRatOSV_interaction.tex'))
###############################################################

###############################################################
# summarize
load(paste0(pathResults, 
	'sobOpp_model1a_global_hier_iccRatOSV_interaction.rda'))
oppMod = modHier
load(paste0(pathResults, 
	'sobState_model1a_global_hier_iccRatOSV_interaction.rda'))
stateMod = modHier
sobMods = lapply(list(stateMod, oppMod), function(x){
	summ=data.frame(fixef(x)[,1:2])
	names(summ) = c('beta','serror')
...
print.xtable(xtable(res, align='llcc', caption=lab),
	...
	size="footnotesize",	
	file=paste0(pathResults, 
		'sob_model1a_global_hier_iccRatOSV_interaction.tex'))
###############################################################