...
###############################################################
# summarize
load(paste0(pathResults, 'sobOpp_model1a_iccRatOnly.rda'))
oppMod = mod
load(paste0(pathResults, 'sobState_model1a_iccRatOnly.rda'))
stateMod = mod
...
print.xtable(xtable(res, align='llcc', caption=lab),
	...
	size="footnotesize",	
	file=paste0(pathResults, 'sob_model1a_iccRatOnly.tex'))
###############################################################

###############################################################
# summarize
load(paste0(pathResults, 'sobOpp_model1a_hier_iccRatOnly.rda'))
oppMod = modHier
load(paste0(pathResults, 'sobState_model1a_hier_iccRatOnly.rda'))
stateMod = modHier
sobMods = lapply(list(stateMod, oppMod), function(x){
	summ=data.frame(fixef(x)[,1:2])
	names(summ) = c('beta','serror')
...
print.xtable(xtable(res, align='llcc', caption=lab),
	...
	hline.after=c(0,0,length(sobVars)*2,length(sobVars)*2),
	size="footnotesize",	
	file=paste0(pathResults, 'sob_model1a_hier_iccRatOnly.tex'))
###############################################################