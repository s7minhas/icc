...
###############################################################
# impute
if(!file.exists(paste0(pathData, 'sobOpp_all.rda'))){
	toImp = data.matrix(yData[,allVars])
	impData = sbgcop.mcmc(Y=toImp, seed=6886, verb=FALSE, nsamp=1000)
	save(impData, file=paste0(pathData, 'sobOpp_all.rda'))
} else { load(paste0(pathData, 'sobOpp_all.rda')) }
...
x = frame[,c('cname','date','icclevel_opp_3')]
cntries = sort(unique(x$cname))
for(cntry in cntries){	
	s=x[x$cname==cntry,]
	delta = diff(s$icclevel_opp_3)
	delta = which(delta>=2)
	if(any(delta >= 2)){
...
# fixes to drc congo
frame$icclevel_opp_3[
	frame$cname=='LIBYAN ARAB JAMAHIRIYA' &
	frame$date == as.Date('2011-08-01')
	] = 2
###############################################################
