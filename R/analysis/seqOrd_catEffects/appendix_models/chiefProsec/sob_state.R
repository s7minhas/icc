...
###############################################################
# create dummy for chief prosectuor and add to model as a global
# effect control
data$chiefProsec = ifelse(data$year >=2012, 1, 0)

#
sobStateVars = c(sobStateVars, 'chiefProsec')
###############################################################
...
save(
  mod, 
  file=paste0(pathResults, 'sobState_chiefProsec_fin.rda'))
###############################################################